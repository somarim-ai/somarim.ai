...
function AINode({ position, glowColor }) {
  ...
  return (
    <mesh ref={mesh} position={position}>
      ...
      <meshBasicMaterial color={glowColor} toneMapped={false} emissive={glowColor} emissiveIntensity={1} />
    </mesh>
  );
}
  ...
  return (
    <group ref={group}>
      {nodes.map((node, index) => (
        <AINode key={index} position={node.position} glowColor={node.glowColor} />
      ))}
    </group>
  );
}
