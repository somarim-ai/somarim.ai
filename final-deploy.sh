...
if ! [ -x "$ZIP_EXECUTABLE" ]; then
    echo "🚨 zip executable not found at $ZIP_EXECUTABLE"
    exit 1
fi
...
(cd backend/lambda && "$ZIP_EXECUTABLE" -r medical-lambda.zip . -x "*.git*" "*.DS_Store*")
